...
from nisyscfg.errors import *
from nisyscfg.types import *


class Library(object):
    def __init__(self, ctypes_library) -> None:
    ...
    def InitializeSession(self, targetName, username, password, language, forcePropertyRefresh, connectTimeoutMsec, expertEnumHandle, sessionHandle) -> Status:
        with self._func_lock:
            if self._InitializeSession_cfunc is None:
                self._InitializeSession_cfunc = self._library.windll.NISysCfgInitializeSession
                self._InitializeSession_cfunc.argtypes = [ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char), Locale, Bool, ctypes.c_uint, EnumExpertHandle, SessionHandle]
                self._InitializeSession_cfunc.restype = Status
    ...
    def CloseHandle(self, syscfgHandle) -> Status:
        with self._func_lock:
            if self._CloseHandle_cfunc is None:
                self._CloseHandle_cfunc = self._library.windll.NISysCfgCloseHandle
                self._CloseHandle_cfunc.argtypes = [ctypes.c_void_p]
                self._CloseHandle_cfunc.restype = Status
        return self._CloseHandle_cfunc(syscfgHandle)

    def GetSystemExperts(self, sessionHandle, expertNames, expertEnumHandle) -> Status:
        with self._func_lock:
            if self._GetSystemExperts_cfunc is None:
                self._GetSystemExperts_cfunc = self._library.windll.NISysCfgGetSystemExperts
                self._GetSystemExperts_cfunc.argtypes = [SessionHandle, ctypes.POINTER(ctypes.c_char), EnumExpertHandle]
                self._GetSystemExperts_cfunc.restype = Status
        return self._GetSystemExperts_cfunc(sessionHandle, expertNames, expertEnumHandle)

    def SetRemoteTimeout(self, sessionHandle, remoteTimeoutMsec) -> Status:
        with self._func_lock:
            if self._SetRemoteTimeout_cfunc is None:
                self._SetRemoteTimeout_cfunc = self._library.windll.NISysCfgSetRemoteTimeout
                self._SetRemoteTimeout_cfunc.argtypes = [SessionHandle, ctypes.c_uint]
                self._SetRemoteTimeout_cfunc.restype = Status
        return self._SetRemoteTimeout_cfunc(sessionHandle, remoteTimeoutMsec)

    def FindHardware(self, sessionHandle, filterMode, filterHandle, expertNames, resourceEnumHandle) -> Status:
        with self._func_lock:
            if self._FindHardware_cfunc is None:
                self._FindHardware_cfunc = self._library.windll.NISysCfgFindHardware
                self._FindHardware_cfunc.argtypes = [SessionHandle, FilterMode, FilterHandle, ctypes.POINTER(ctypes.c_char), EnumResourceHandle]
                self._FindHardware_cfunc.restype = Status
        return self._FindHardware_cfunc(sessionHandle, filterMode, filterHandle, expertNames, resourceEnumHandle)

    def FindSystems(self, sessionHandle, deviceClass, detectOnlineSystems, cacheMode, findOutputMode, timeoutMsec, onlyInstallableSystems, systemEnumHandle) -> Status:
        with self._func_lock:
            if self._FindSystems_cfunc is None:
                self._FindSystems_cfunc = self._library.windll.NISysCfgFindSystems
                self._FindSystems_cfunc.argtypes = [SessionHandle, ctypes.POINTER(ctypes.c_char), Bool, IncludeCachedResults, SystemNameFormat, ctypes.c_uint, Bool, EnumSystemHandle]
                self._FindSystems_cfunc.restype = Status
    ...
    def SelfTestHardware(self, resourceHandle, mode, detailedResult) -> Status:
        with self._func_lock:
            if self._SelfTestHardware_cfunc is None:
                self._SelfTestHardware_cfunc = self._library.windll.NISysCfgSelfTestHardware
                self._SelfTestHardware_cfunc.argtypes = [ResourceHandle, ctypes.c_uint, ctypes.POINTER(ctypes.POINTER(ctypes.c_char))]
                self._SelfTestHardware_cfunc.restype = Status
        return self._SelfTestHardware_cfunc(resourceHandle, mode, detailedResult)

    def SelfCalibrateHardware(self, resourceHandle, detailedResult) -> Status:
        with self._func_lock:
            if self._SelfCalibrateHardware_cfunc is None:
                ...
                self._SelfCalibrateHardware_cfunc.argtypes = [ResourceHandle, ctypes.POINTER(ctypes.POINTER(ctypes.c_char))]
                self._SelfCalibrateHardware_cfunc.restype = Status
        return self._SelfCalibrateHardware_cfunc(resourceHandle, detailedResult)

    def ResetHardware(self, resourceHandle, mode) -> Status:
        with self._func_lock:
            if self._ResetHardware_cfunc is None:
                self._ResetHardware_cfunc = self._library.windll.NISysCfgResetHardware
                self._ResetHardware_cfunc.argtypes = [ResourceHandle, ctypes.c_uint]
                self._ResetHardware_cfunc.restype = Status
        return self._ResetHardware_cfunc(resourceHandle, mode)

    def RenameResource(self, resourceHandle, newName, overwriteConflict, updateDependencies, nameAlreadyExisted, overwrittenResourceHandle) -> Status:
        with self._func_lock:
            if self._RenameResource_cfunc is None:
                self._RenameResource_cfunc = self._library.windll.NISysCfgRenameResource
                self._RenameResource_cfunc.argtypes = [ResourceHandle, ctypes.POINTER(ctypes.c_char), Bool, Bool, ctypes.POINTER(ctypes.c_int), ResourceHandle]
                self._RenameResource_cfunc.restype = Status
    ...
    def DeleteResource(self, resourceHandle, mode, dependentItemsDeleted, detailedResult) -> Status:
        with self._func_lock:
            if self._DeleteResource_cfunc is None:
                self._DeleteResource_cfunc = self._library.windll.NISysCfgDeleteResource
                self._DeleteResource_cfunc.argtypes = [ResourceHandle, DeleteValidationMode, ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.POINTER(ctypes.c_char))]
                self._DeleteResource_cfunc.restype = Status
        return self._DeleteResource_cfunc(resourceHandle, mode, dependentItemsDeleted, detailedResult)

    def GetResourceProperty(self, resourceHandle, propertyID, value) -> Status:
        with self._func_lock:
            if self._GetResourceProperty_cfunc is None:
                ...
                self._GetResourceProperty_cfunc.argtypes = [ResourceHandle, ctypes.c_uint, ctypes.c_void_p]
                self._GetResourceProperty_cfunc.restype = Status
        return self._GetResourceProperty_cfunc(resourceHandle, propertyID, value)

    def SetResourceProperty(self, resourceHandle, propertyID, args) -> Status:
        with self._func_lock:
            if self._SetResourceProperty_cfunc is None:
                self._SetResourceProperty_cfunc = self._library.cdll.NISysCfgSetResourceProperty
                self._SetResourceProperty_cfunc.restype = Status
        return self._SetResourceProperty_cfunc(resourceHandle, propertyID, args)

    def SetResourcePropertyWithType(self, resourceHandle, propertyID, propertyType, args) -> Status:
        with self._func_lock:
            if self._SetResourcePropertyWithType_cfunc is None:
                ...
                self._SetResourcePropertyWithType_cfunc.restype = Status
        return self._SetResourcePropertyWithType_cfunc(resourceHandle, propertyID, propertyType, args)

    def SetResourcePropertyV(self, resourceHandle, propertyID, args) -> Status:
        with self._func_lock:
            if self._SetResourcePropertyV_cfunc is None:
                ...
                self._SetResourcePropertyV_cfunc.restype = Status
        return self._SetResourcePropertyV_cfunc(resourceHandle, propertyID, args)

    def SetResourcePropertyWithTypeV(self, resourceHandle, propertyID, propertyType, args) -> Status:
        with self._func_lock:
            if self._SetResourcePropertyWithTypeV_cfunc is None:
                ...
                self._SetResourcePropertyWithTypeV_cfunc.restype = Status
        return self._SetResourcePropertyWithTypeV_cfunc(resourceHandle, propertyID, propertyType, args)

    def GetResourceIndexedProperty(self, resourceHandle, propertyID, index, value) -> Status:
        with self._func_lock:
            if self._GetResourceIndexedProperty_cfunc is None:
                ...
                self._GetResourceIndexedProperty_cfunc.argtypes = [ResourceHandle, ctypes.c_uint, ctypes.c_uint, ctypes.c_void_p]
                self._GetResourceIndexedProperty_cfunc.restype = Status
        return self._GetResourceIndexedProperty_cfunc(resourceHandle, propertyID, index, value)

    def SaveResourceChanges(self, resourceHandle, changesRequireRestart, detailedResult) -> Status:
        with self._func_lock:
    ...
    def SetFilterPropertyWithTypeV(self, filterHandle, propertyID, propertyType, args) -> Status:
        with self._func_lock:
            if self._SetFilterPropertyWithTypeV_cfunc is None:
                ...
                self._SetFilterPropertyWithTypeV_cfunc.restype = Status
        return self._SetFilterPropertyWithTypeV_cfunc(filterHandle, propertyID, propertyType, args)

    def UpgradeFirmwareFromFile(self, resourceHandle, firmwareFile, autoStopTasks, alwaysOverwrite, waitForOperationToFinish, firmwareStatus, detailedResult) -> Status:
        with self._func_lock:
            if self._UpgradeFirmwareFromFile_cfunc is None:
                ...
                self._UpgradeFirmwareFromFile_cfunc.argtypes = [ResourceHandle, ctypes.POINTER(ctypes.c_char), Bool, Bool, Bool, ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.POINTER(ctypes.c_char))]
                self._UpgradeFirmwareFromFile_cfunc.restype = Status
    ...
    def UpgradeFirmwareVersion(self, resourceHandle, firmwareVersion, autoStopTasks, alwaysOverwrite, waitForOperationToFinish, firmwareStatus, detailedResult) -> Status:
        with self._func_lock:
            if self._UpgradeFirmwareVersion_cfunc is None:
                ...
                self._UpgradeFirmwareVersion_cfunc.argtypes = [ResourceHandle, ctypes.POINTER(ctypes.c_char), Bool, Bool, Bool, ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.POINTER(ctypes.c_char))]
                self._UpgradeFirmwareVersion_cfunc.restype = Status
    ...
    def EraseFirmware(self, resourceHandle, autoStopTasks, firmwareStatus, detailedResult) -> Status:
        with self._func_lock:
            if self._EraseFirmware_cfunc is None:
                self._EraseFirmware_cfunc = self._library.windll.NISysCfgEraseFirmware
                self._EraseFirmware_cfunc.argtypes = [ResourceHandle, Bool, ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.POINTER(ctypes.c_char))]
                self._EraseFirmware_cfunc.restype = Status
        return self._EraseFirmware_cfunc(resourceHandle, autoStopTasks, firmwareStatus, detailedResult)

    def CheckFirmwareStatus(self, resourceHandle, percentComplete, firmwareStatus, detailedResult) -> Status:
        with self._func_lock:
            if self._CheckFirmwareStatus_cfunc is None:
                ...
                self._CheckFirmwareStatus_cfunc.argtypes = [ResourceHandle, ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.POINTER(ctypes.c_char))]
                self._CheckFirmwareStatus_cfunc.restype = Status
        return self._CheckFirmwareStatus_cfunc(resourceHandle, percentComplete, firmwareStatus, detailedResult)

    def Format(self, sessionHandle, forceSafeMode, restartAfterFormat, fileSystem, networkSettings, timeoutMsec) -> Status:
        with self._func_lock:
            if self._Format_cfunc is None:
                self._Format_cfunc = self._library.windll.NISysCfgFormat
                self._Format_cfunc.argtypes = [SessionHandle, Bool, Bool, FileSystemMode, NetworkInterfaceSettings, ctypes.c_uint]
                self._Format_cfunc.restype = Status
        return self._Format_cfunc(sessionHandle, forceSafeMode, restartAfterFormat, fileSystem, networkSettings, timeoutMsec)

    def FormatWithBaseSystemImage(self, sessionHandle, autoRestart, fileSystem, networkSettings, systemImageID, systemImageVersion, timeoutMsec) -> Status:
        with self._func_lock:
            if self._FormatWithBaseSystemImage_cfunc is None:
                ...
                self._FormatWithBaseSystemImage_cfunc.argtypes = [SessionHandle, Bool, FileSystemMode, NetworkInterfaceSettings, ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char), ctypes.c_uint]
                self._FormatWithBaseSystemImage_cfunc.restype = Status
    ...
    def Restart(self, sessionHandle, waitForRestartToFinish, installMode, flushDNS, timeoutMsec, newIpAddress) -> Status:
        with self._func_lock:
            if self._Restart_cfunc is None:
                self._Restart_cfunc = self._library.windll.NISysCfgRestart
                self._Restart_cfunc.argtypes = [SessionHandle, Bool, Bool, Bool, ctypes.c_uint, ctypes.POINTER(ctypes.c_char)]
                self._Restart_cfunc.restype = Status
        return self._Restart_cfunc(sessionHandle, waitForRestartToFinish, installMode, flushDNS, timeoutMsec, newIpAddress)

    def GetAvailableSoftwareComponents(self, sessionHandle, itemTypes, componentEnumHandle) -> Status:
        with self._func_lock:
            if self._GetAvailableSoftwareComponents_cfunc is None:
                ...
                self._GetAvailableSoftwareComponents_cfunc.argtypes = [SessionHandle, IncludeComponentTypes, EnumSoftwareComponentHandle]
                self._GetAvailableSoftwareComponents_cfunc.restype = Status
        return self._GetAvailableSoftwareComponents_cfunc(sessionHandle, itemTypes, componentEnumHandle)

    def GetAvailableSoftwareSets(self, sessionHandle, setEnumHandle) -> Status:
        with self._func_lock:
            if self._GetAvailableSoftwareSets_cfunc is None:
                ...
                self._GetAvailableSoftwareSets_cfunc.argtypes = [SessionHandle, EnumSoftwareSetHandle]
                self._GetAvailableSoftwareSets_cfunc.restype = Status
        return self._GetAvailableSoftwareSets_cfunc(sessionHandle, setEnumHandle)

    def GetFilteredSoftwareComponents(self, repositoryPath, deviceClass, operatingSystem, productID, itemTypes, componentEnumHandle) -> Status:
        with self._func_lock:
            if self._GetFilteredSoftwareComponents_cfunc is None:
                ...
                self._GetFilteredSoftwareComponents_cfunc.argtypes = [ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char), ctypes.c_uint, IncludeComponentTypes, EnumSoftwareComponentHandle]
                self._GetFilteredSoftwareComponents_cfunc.restype = Status
    ...
    def GetFilteredSoftwareSets(self, repositoryPath, deviceClass, operatingSystem, productID, setEnumHandle) -> Status:
        with self._func_lock:
            if self._GetFilteredSoftwareSets_cfunc is None:
                ...
                self._GetFilteredSoftwareSets_cfunc.argtypes = [ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char), ctypes.c_uint, EnumSoftwareSetHandle]
                self._GetFilteredSoftwareSets_cfunc.restype = Status
        return self._GetFilteredSoftwareSets_cfunc(repositoryPath, deviceClass, operatingSystem, productID, setEnumHandle)

    def GetFilteredBaseSystemImages(self, repositoryPath, deviceClass, operatingSystem, productID, systemImageEnumHandle) -> Status:
        with self._func_lock:
            if self._GetFilteredBaseSystemImages_cfunc is None:
                ...
                self._GetFilteredBaseSystemImages_cfunc.argtypes = [ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char), ctypes.c_uint, EnumSoftwareComponentHandle]
                self._GetFilteredBaseSystemImages_cfunc.restype = Status
    ...
    def GetInstalledSoftwareComponents(self, sessionHandle, itemTypes, cached, componentEnumHandle) -> Status:
        with self._func_lock:
            if self._GetInstalledSoftwareComponents_cfunc is None:
                ...
                self._GetInstalledSoftwareComponents_cfunc.argtypes = [SessionHandle, IncludeComponentTypes, Bool, EnumSoftwareComponentHandle]
                self._GetInstalledSoftwareComponents_cfunc.restype = Status
        return self._GetInstalledSoftwareComponents_cfunc(sessionHandle, itemTypes, cached, componentEnumHandle)

    def GetInstalledSoftwareSet(self, sessionHandle, cached, setHandle) -> Status:
        with self._func_lock:
            if self._GetInstalledSoftwareSet_cfunc is None:
                ...
                self._GetInstalledSoftwareSet_cfunc.argtypes = [SessionHandle, Bool, SoftwareSetHandle]
                self._GetInstalledSoftwareSet_cfunc.restype = Status
        return self._GetInstalledSoftwareSet_cfunc(sessionHandle, cached, setHandle)

    def GetSystemImageAsFolder(self, sessionHandle, destinationFolder, encryptionPassphrase, overwriteIfExists, installedSoftwareOnly, autoRestart) -> Status:
        with self._func_lock:
            if self._GetSystemImageAsFolder_cfunc is None:
                ...
                self._GetSystemImageAsFolder_cfunc.argtypes = [SessionHandle, ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char), Bool, Bool, Bool]
                self._GetSystemImageAsFolder_cfunc.restype = Status
    ...
    def GetSystemImageAsFolder2(self, sessionHandle, autoRestart, destinationFolder, encryptionPassphrase, numBlacklistEntries, blacklistFilesDirectories, overwriteIfExists, installedSoftwareOnly) -> Status:
        with self._func_lock:
            if self._GetSystemImageAsFolder2_cfunc is None:
                ...
                self._GetSystemImageAsFolder2_cfunc.argtypes = [SessionHandle, Bool, ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char), ctypes.c_uint, ctypes.POINTER(ctypes.POINTER(ctypes.c_char)), Bool, Bool]
                self._GetSystemImageAsFolder2_cfunc.restype = Status
    ...
    def CreateSystemImageAsFolder(self, sessionHandle, imageTitle, imageID, imageVersion, imageDescription, autoRestart, destinationFolder, encryptionPassphrase, numBlacklistEntries, blacklistFilesDirectories, overwriteIfExists) -> Status:
        with self._func_lock:
            if self._CreateSystemImageAsFolder_cfunc is None:
                ...
                self._CreateSystemImageAsFolder_cfunc.restype = Status
    ...
    def SetSystemImageFromFolder(self, sessionHandle, sourceFolder, encryptionPassphrase, autoRestart, originalSystemOnly) -> Status:
        with self._func_lock:
            if self._SetSystemImageFromFolder_cfunc is None:
                ...
                self._SetSystemImageFromFolder_cfunc.argtypes = [SessionHandle, ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char), Bool, Bool]
                self._SetSystemImageFromFolder_cfunc.restype = Status
    ...
    def SetSystemImageFromFolder2(self, sessionHandle, autoRestart, sourceFolder, encryptionPassphrase, numBlacklistEntries, blacklistFilesDirectories, originalSystemOnly, networkSettings) -> Status:
        with self._func_lock:
            if self._SetSystemImageFromFolder2_cfunc is None:
                ...
                self._SetSystemImageFromFolder2_cfunc.argtypes = [SessionHandle, Bool, ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char), ctypes.c_uint, ctypes.POINTER(ctypes.POINTER(ctypes.c_char)), Bool, NetworkInterfaceSettings]
                self._SetSystemImageFromFolder2_cfunc.restype = Status
    ...
    def InstallAll(self, sessionHandle, autoRestart, deselectConflicts, installedComponentEnumHandle, brokenDependencyEnumHandle) -> Status:
        with self._func_lock:
            if self._InstallAll_cfunc is None:
                self._InstallAll_cfunc = self._library.windll.NISysCfgInstallAll
                self._InstallAll_cfunc.argtypes = [SessionHandle, Bool, Bool, EnumSoftwareComponentHandle, EnumDependencyHandle]
                self._InstallAll_cfunc.restype = Status
    ...
    def InstallUninstallComponents(self, sessionHandle, autoRestart, autoSelectDependencies, componentToInstallEnumHandle, numComponentsToUninstall, componentIDsToUninstall, brokenDependencyEnumHandle) -> Status:
        with self._func_lock:
            if self._InstallUninstallComponents_cfunc is None:
                ...
                self._InstallUninstallComponents_cfunc.argtypes = [SessionHandle, Bool, Bool, EnumSoftwareComponentHandle, ctypes.c_uint, ctypes.POINTER(ctypes.POINTER(ctypes.c_char)), EnumDependencyHandle]
                self._InstallUninstallComponents_cfunc.restype = Status
    ...
    def InstallUninstallComponents2(self, sessionHandle, autoRestart, autoSelectDependencies, autoSelectRecommends, componentToInstallEnumHandle, numComponentsToUninstall, componentIDsToUninstall, brokenDependencyEnumHandle) -> Status:
        with self._func_lock:
            if self._InstallUninstallComponents2_cfunc is None:
                ...
                self._InstallUninstallComponents2_cfunc.argtypes = [SessionHandle, Bool, Bool, Bool, EnumSoftwareComponentHandle, ctypes.c_uint, ctypes.POINTER(ctypes.POINTER(ctypes.c_char)), EnumDependencyHandle]
                self._InstallUninstallComponents2_cfunc.restype = Status
    ...
    def InstallSoftwareSet(self, sessionHandle, autoRestart, softwareSetID, version, addonEnumHandle, brokenDependencyEnumHandle) -> Status:
        with self._func_lock:
            if self._InstallSoftwareSet_cfunc is None:
                self._InstallSoftwareSet_cfunc = self._library.windll.NISysCfgInstallSoftwareSet
                self._InstallSoftwareSet_cfunc.argtypes = [SessionHandle, Bool, ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char), EnumSoftwareComponentHandle, EnumDependencyHandle]
                self._InstallSoftwareSet_cfunc.restype = Status
    ...
    def InstallStartup(self, sessionHandle, autoRestart, startupEnumHandle, uninstallConflicts, installedComponentEnumHandle, uninstalledComponentEnumHandle, brokenDependencyEnumHandle) -> Status:
        with self._func_lock:
            if self._InstallStartup_cfunc is None:
                self._InstallStartup_cfunc = self._library.windll.NISysCfgInstallStartup
                self._InstallStartup_cfunc.argtypes = [SessionHandle, Bool, EnumSoftwareComponentHandle, Bool, EnumSoftwareComponentHandle, EnumSoftwareComponentHandle, EnumDependencyHandle]
                self._InstallStartup_cfunc.restype = Status
    ...
    def UninstallAll(self, sessionHandle, autoRestart) -> Status:
        with self._func_lock:
            if self._UninstallAll_cfunc is None:
                self._UninstallAll_cfunc = self._library.windll.NISysCfgUninstallAll
                self._UninstallAll_cfunc.argtypes = [SessionHandle, Bool]
                self._UninstallAll_cfunc.restype = Status
        return self._UninstallAll_cfunc(sessionHandle, autoRestart)

    def GetSoftwareFeeds(self, sessionHandle, feedEnumHandle) -> Status:
        with self._func_lock:
            if self._GetSoftwareFeeds_cfunc is None:
                self._GetSoftwareFeeds_cfunc = self._library.windll.NISysCfgGetSoftwareFeeds
                self._GetSoftwareFeeds_cfunc.argtypes = [SessionHandle, EnumSoftwareFeedHandle]
                self._GetSoftwareFeeds_cfunc.restype = Status
        return self._GetSoftwareFeeds_cfunc(sessionHandle, feedEnumHandle)

    def AddSoftwareFeed(self, sessionHandle, feedName, uri, enabled, trusted) -> Status:
        with self._func_lock:
            if self._AddSoftwareFeed_cfunc is None:
                self._AddSoftwareFeed_cfunc = self._library.windll.NISysCfgAddSoftwareFeed
                self._AddSoftwareFeed_cfunc.argtypes = [SessionHandle, ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char), Bool, Bool]
                self._AddSoftwareFeed_cfunc.restype = Status
        return self._AddSoftwareFeed_cfunc(sessionHandle, feedName, uri, enabled, trusted)

    def ModifySoftwareFeed(self, sessionHandle, feedName, newFeedName, uri, enabled, trusted) -> Status:
        with self._func_lock:
            if self._ModifySoftwareFeed_cfunc is None:
                self._ModifySoftwareFeed_cfunc = self._library.windll.NISysCfgModifySoftwareFeed
                self._ModifySoftwareFeed_cfunc.argtypes = [SessionHandle, ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char), Bool, Bool]
                self._ModifySoftwareFeed_cfunc.restype = Status
        return self._ModifySoftwareFeed_cfunc(sessionHandle, feedName, newFeedName, uri, enabled, trusted)

    def RemoveSoftwareFeed(self, sessionHandle, feedName) -> Status:
        with self._func_lock:
            if self._RemoveSoftwareFeed_cfunc is None:
                self._RemoveSoftwareFeed_cfunc = self._library.windll.NISysCfgRemoveSoftwareFeed
                self._RemoveSoftwareFeed_cfunc.argtypes = [SessionHandle, ctypes.POINTER(ctypes.c_char)]
                self._RemoveSoftwareFeed_cfunc.restype = Status
        return self._RemoveSoftwareFeed_cfunc(sessionHandle, feedName)

    def ChangeAdministratorPassword(self, sessionHandle, newPassword) -> Status:
        with self._func_lock:
            if self._ChangeAdministratorPassword_cfunc is None:
                ...
                self._ChangeAdministratorPassword_cfunc.argtypes = [SessionHandle, ctypes.POINTER(ctypes.c_char)]
                self._ChangeAdministratorPassword_cfunc.restype = Status
        return self._ChangeAdministratorPassword_cfunc(sessionHandle, newPassword)

    def ExportConfiguration(self, sessionHandle, destinationFile, expertNames, overwriteIfExists) -> Status:
        with self._func_lock:
            if self._ExportConfiguration_cfunc is None:
                ...
                self._ExportConfiguration_cfunc.argtypes = [SessionHandle, ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char), Bool]
                self._ExportConfiguration_cfunc.restype = Status
        return self._ExportConfiguration_cfunc(sessionHandle, destinationFile, expertNames, overwriteIfExists)

    def ImportConfiguration(self, sessionHandle, sourceFile, expertNames, importMode, detailedResult) -> Status:
        with self._func_lock:
            if self._ImportConfiguration_cfunc is None:
                ...
                self._ImportConfiguration_cfunc.argtypes = [SessionHandle, ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char), ImportMode, ctypes.POINTER(ctypes.POINTER(ctypes.c_char))]
                self._ImportConfiguration_cfunc.restype = Status
        return self._ImportConfiguration_cfunc(sessionHandle, sourceFile, expertNames, importMode, detailedResult)

    def GenerateMAXReport(self, sessionHandle, outputFilename, reportType, overwriteIfExists) -> Status:
        with self._func_lock:
            if self._GenerateMAXReport_cfunc is None:
                self._GenerateMAXReport_cfunc = self._library.windll.NISysCfgGenerateMAXReport
                self._GenerateMAXReport_cfunc.argtypes = [SessionHandle, ctypes.POINTER(ctypes.c_char), ReportType, Bool]
                self._GenerateMAXReport_cfunc.restype = Status
        return self._GenerateMAXReport_cfunc(sessionHandle, outputFilename, reportType, overwriteIfExists)

    def CreateComponentsEnum(self, componentEnumHandle) -> Status:
        with self._func_lock:
            if self._CreateComponentsEnum_cfunc is None:
                ...
                self._CreateComponentsEnum_cfunc.argtypes = [EnumSoftwareComponentHandle]
                self._CreateComponentsEnum_cfunc.restype = Status
        return self._CreateComponentsEnum_cfunc(componentEnumHandle)

    def AddComponentToEnum(self, componentEnumHandle, ID, version, mode) -> Status:
        with self._func_lock:
            if self._AddComponentToEnum_cfunc is None:
                self._AddComponentToEnum_cfunc = self._library.windll.NISysCfgAddComponentToEnum
                self._AddComponentToEnum_cfunc.argtypes = [EnumSoftwareComponentHandle, ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char), VersionSelectionMode]
                self._AddComponentToEnum_cfunc.restype = Status
        return self._AddComponentToEnum_cfunc(componentEnumHandle, ID, version, mode)

    def FreeDetailedString(self, str) -> Status:
        with self._func_lock:
            if self._FreeDetailedString_cfunc is None:
                self._FreeDetailedString_cfunc = self._library.windll.NISysCfgFreeDetailedString
                self._FreeDetailedString_cfunc.argtypes = [ctypes.POINTER(ctypes.c_char)]
                self._FreeDetailedString_cfunc.restype = Status
        return self._FreeDetailedString_cfunc(str)

    def NextResource(self, sessionHandle, resourceEnumHandle, resourceHandle) -> Status:
        with self._func_lock:
            if self._NextResource_cfunc is None:
                self._NextResource_cfunc = self._library.windll.NISysCfgNextResource
                self._NextResource_cfunc.argtypes = [SessionHandle, EnumResourceHandle, ResourceHandle]
                self._NextResource_cfunc.restype = Status
        return self._NextResource_cfunc(sessionHandle, resourceEnumHandle, resourceHandle)

    def NextSystemInfo(self, systemEnumHandle, system) -> Status:
        with self._func_lock:
            if self._NextSystemInfo_cfunc is None:
                self._NextSystemInfo_cfunc = self._library.windll.NISysCfgNextSystemInfo
                self._NextSystemInfo_cfunc.argtypes = [EnumSystemHandle, ctypes.POINTER(ctypes.c_char)]
                self._NextSystemInfo_cfunc.restype = Status
        return self._NextSystemInfo_cfunc(systemEnumHandle, system)

    def NextExpertInfo(self, expertEnumHandle, expertName, displayName, version) -> Status:
        with self._func_lock:
            if self._NextExpertInfo_cfunc is None:
                self._NextExpertInfo_cfunc = self._library.windll.NISysCfgNextExpertInfo
                self._NextExpertInfo_cfunc.argtypes = [EnumExpertHandle, ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char)]
                self._NextExpertInfo_cfunc.restype = Status
        return self._NextExpertInfo_cfunc(expertEnumHandle, expertName, displayName, version)

    def NextComponentInfo(self, componentEnumHandle, ID, version, title, itemType, detailedDescription) -> Status:
        with self._func_lock:
            if self._NextComponentInfo_cfunc is None:
                self._NextComponentInfo_cfunc = self._library.windll.NISysCfgNextComponentInfo
                self._NextComponentInfo_cfunc.argtypes = [EnumSoftwareComponentHandle, ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.POINTER(ctypes.c_char))]
                self._NextComponentInfo_cfunc.restype = Status
        return self._NextComponentInfo_cfunc(componentEnumHandle, ID, version, title, itemType, detailedDescription)

    def NextSoftwareSet(self, setEnumHandle, setHandle) -> Status:
        with self._func_lock:
            if self._NextSoftwareSet_cfunc is None:
                self._NextSoftwareSet_cfunc = self._library.windll.NISysCfgNextSoftwareSet
                self._NextSoftwareSet_cfunc.argtypes = [EnumSoftwareSetHandle, SoftwareSetHandle]
                self._NextSoftwareSet_cfunc.restype = Status
        return self._NextSoftwareSet_cfunc(setEnumHandle, setHandle)

    def GetSoftwareSetInfo(self, setHandle, itemTypes, includeAddOnDeps, ID, version, title, setType, detailedDescription, addOnEnumHandle, itemEnumHandle) -> Status:
        with self._func_lock:
            if self._GetSoftwareSetInfo_cfunc is None:
                self._GetSoftwareSetInfo_cfunc = self._library.windll.NISysCfgGetSoftwareSetInfo
                self._GetSoftwareSetInfo_cfunc.argtypes = [SoftwareSetHandle, IncludeComponentTypes, Bool, ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.POINTER(ctypes.c_char)), EnumSoftwareComponentHandle, EnumSoftwareComponentHandle]
                self._GetSoftwareSetInfo_cfunc.restype = Status
    ...
    def NextDependencyInfo(self, dependencyEnumHandle, dependerID, dependerVersion, dependerTitle, dependerDetailedDescription, dependeeID, dependeeVersion, dependeeTitle, dependeeDetailedDescription) -> Status:
        with self._func_lock:
            if self._NextDependencyInfo_cfunc is None:
                self._NextDependencyInfo_cfunc = self._library.windll.NISysCfgNextDependencyInfo
                self._NextDependencyInfo_cfunc.argtypes = [EnumDependencyHandle, ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.POINTER(ctypes.c_char)), ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.POINTER(ctypes.c_char))]
                self._NextDependencyInfo_cfunc.restype = Status
    ...
    def NextSoftwareFeed(self, feedEnumHandle, feedName, uri, enabled, trusted) -> Status:
        with self._func_lock:
            if self._NextSoftwareFeed_cfunc is None:
                self._NextSoftwareFeed_cfunc = self._library.windll.NISysCfgNextSoftwareFeed
                self._NextSoftwareFeed_cfunc.argtypes = [EnumSoftwareFeedHandle, ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.c_int)]
                self._NextSoftwareFeed_cfunc.restype = Status
        return self._NextSoftwareFeed_cfunc(feedEnumHandle, feedName, uri, enabled, trusted)

    def ResetEnumeratorGetCount(self, enumHandle, count) -> Status:
        with self._func_lock:
            if self._ResetEnumeratorGetCount_cfunc is None:
                ...
                self._ResetEnumeratorGetCount_cfunc.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.c_uint)]
                self._ResetEnumeratorGetCount_cfunc.restype = Status
        return self._ResetEnumeratorGetCount_cfunc(enumHandle, count)

    def GetStatusDescription(self, sessionHandle, status, detailedDescription) -> Status:
        with self._func_lock:
            if self._GetStatusDescription_cfunc is None:
                ...
                self._GetStatusDescription_cfunc.argtypes = [SessionHandle, Status, ctypes.POINTER(ctypes.POINTER(ctypes.c_char))]
                self._GetStatusDescription_cfunc.restype = Status
        return self._GetStatusDescription_cfunc(sessionHandle, status, detailedDescription)

    def TimestampFromValues(self, secondsSinceEpoch1970, fractionalSeconds, timestamp) -> Status:
        with self._func_lock:
            if self._TimestampFromValues_cfunc is None:
                ...
                self._TimestampFromValues_cfunc.argtypes = [UInt64, ctypes.c_double, ctypes.POINTER(TimestampUTC)]
                self._TimestampFromValues_cfunc.restype = Status
        return self._TimestampFromValues_cfunc(secondsSinceEpoch1970, fractionalSeconds, timestamp)

    def ValuesFromTimestamp(self, timestamp, secondsSinceEpoch1970, fractionalSeconds) -> Status:
        with self._func_lock:
            if self._ValuesFromTimestamp_cfunc is None:
                ...
                self._ValuesFromTimestamp_cfunc.argtypes = [TimestampUTC, ctypes.POINTER(UInt64), ctypes.POINTER(ctypes.c_double)]
                self._ValuesFromTimestamp_cfunc.restype = Status
